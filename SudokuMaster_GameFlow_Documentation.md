...
stateDiagram-v2
    ...
    state GameScene {
        [*] --> BoardInitialization
        BoardInitialization --> ValidationSetup : SudokuBoard.Start():75
        ValidationSetup --> CellCreation : CreateGrid()
        CellCreation --> ImmediateStrategy : InitializeValidationContext():88
        ImmediateStrategy --> GameplayLoop
        
        state GameplayLoop {
            [*] --> WaitingForInput
            WaitingForInput --> CellSelected : OnPointerDown():114
            CellSelected --> NumberPlacement : SetSquareNumber()
            NumberPlacement --> ImmediateValidation : ProcessMove():302
            ImmediateValidation --> AudioFeedback : ShouldPlayAudio():132
            AudioFeedback --> LifeSystem : ShouldUpdateLivesSystem():131
...
stateDiagram-v2
    ...
    state Hypothesis {
        ...
        note right of OrangeStyling : Hypothesis_Color:37\nHypothesis_Text_Color:38
    }
    ...
    note right of Hypothesis : SetAsHypothesis():594-598\nVisual distinction from confirmed numbers
    note right of ValidationRequested : ValidateCompleteBoard():60\nComprehensive board analysis
```
...
graph LR
    subgraph "Performance Features"
        CC[Cell Caching<br/>Dictionary<int, SudokuCell><br/>Fast O(1) access]
        BU[Batched Updates<br/>BatchProcessCellChanges():198<br/>Reduced UI calls]
        OP[Object Pooling<br/>Reusable UI elements<br/>Memory efficient]
        CD[Cache Invalidation<br/>MarkCacheDirty():192<br/>Automatic refresh]
    end
    
    subgraph "ValidationContext.cs"
        IC[InitializeCellCache():162-179]
        GCF[GetCellFast():181-190]
    end
