    ...
    public partial class ALSTechGen: AnalyzerBaseV2{
     //  static public Bit81[]    pConnectedCells{ get{ return AnalyzerBaseV2.ConnectedCells; } }
        ...
        private string ALS_Wing_SolResult( UCell P0, List<LinkCellALS> LK_CeALSs, int noX, Bit81 elmB ){
            int noXB = 1<<noX;
            //alex  P0.Set_CellColorBkgColor_noBit(P0.FreeB.DifSet(noXB), AttCr3, SolBkCr4);
            //alex P0.Set_CellColorBkgColor_noBit(noXB, AttCr, SolBkCr4 );

            int kx = 0;
            string msg = "";
            foreach( var LK in LK_CeALSs){
                /*alex Color Cr = _ColorsLst[++kx];
            ...
            foreach( var P in pBDL.Where(p=>p.FreeB>0 && p.ECrLst!=null) ){
                ...
                //alex ????  var G = P.ECrLst.GroupBy(p=>p.ClrCellBkg).ToList();
                //alex ???? if( G.Count(q=> ((Color)q.Key)!=Colors.Black) >= 2 ) overlapB.BPSet(P.rc);
            }
