...
namespace GNPXcore{
    public partial class ALSTechGen: AnalyzerBaseV2{
        ...
        private void _DeathBlossom_SolResult( List<LinkCellALS> LKCAsol, UCell SC, int no, Bit81 E, bool connected=false ){
            ...
            //alex Color cr = _ColorsLst[0];////Colors.Gold;
            //alex SC.Set_CellColorBkgColor_noBit(SC.FreeB,AttCr3,cr);
            ...
            foreach( var LK in LKCAsol ){
                int noB2 = 1<<LK.nRCC;
                //alex cr = _ColorsLst[++k];
                LK.ALS.UCellLst.ForEach( p=> { 
                    UCell P = pBDL[p.rc];
                    //alex P.Set_CellColorBkgColor_noBit(noB,AttCr,cr);
                    //alex P.Set_CellColorBkgColor_noBit(noB2,AttCr3,cr);
