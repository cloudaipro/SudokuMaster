...
namespace GNPXcore{
    public partial class ALSTechGen: AnalyzerBaseV2{
        ...
        private void _SolResult_ALSChain( int Eno, Bit81 B81rc, Stack<UALSPair> SolStack ){    
            ...
            if( SolInfoB ){
                int noB=0;
                //alex Color cr, clrBlk = Colors.Black;
                ...
                foreach( var LKA in SSrev ){
                    noB = (1<<LKA.nRCC);
                    UALS UA = LKA.ALSpre;
                    //alex cr = _ColorsLst[ (nc++)%_ColorsLst.Length ]; 

                    //UA.UCellLst.IE_SetNoBBgColor( noB, AttCr, cr );
                    //alex  UA.UCellLst.IE_SetNoBBgColor( pBDL, 0, clrBlk, cr );
                ...
                //alex LKB.ALSnxt.UCellLst.IE_SetNoBBgColor( pBDL, 0, AttCr, cr );
