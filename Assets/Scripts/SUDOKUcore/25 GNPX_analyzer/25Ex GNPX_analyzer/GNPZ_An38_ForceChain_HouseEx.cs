		...
		private  IEnumerable<bool> _ForceChainHouseDispEx( Bit81[] multiPathB81, Bit81[] sTrue, int hs0, int no0 ){
			...
			for( int noX=0; noX<9; noX++ ){	
				...
				foreach( var rc in sTrue[noX].IEGet_rc() ){
					...
					if( SolInfoB ){
						//alex PX.Set_CellColorBkgColor_noBit( 1<<noX, Colors.Red , Colors.LightGreen );
						//alex PX.Set_CellDigitsColorRev_noBit( elm, Colors.Red );
						...
						foreach( var P in pBDL.IEGetCellInHouse(hs0,1<<no0) ){
							...
							//alex if( ForceChain_Option!="ForceL3" ) P.Set_CellColorBkgColor_noBit( 1<<no0, Colors.Green , Colors.Yellow );
						}
