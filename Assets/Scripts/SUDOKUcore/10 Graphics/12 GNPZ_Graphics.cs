    ...
    public class GNPZ_Graphics
    {
        ...
        public RenderTargetBitmap CreateCellImage(UCell P, bool candDisp)
        {
            ...
            using (var DC = drawVisual.RenderOpen())
            {
                ...
                if (P.FixedNo > 0)
                { //When the digit is true, the "confirmed color" is used.)
                ...
                if (!(ECrLst is null))
                {
                    foreach (int no in P.FreeB.IEGet_BtoNo())
                    {
                        ...
                        if (!(EC is null))
                        { // --- draw the background color of the digit   
        ...
        public void GBPatternPaint(Canvas can, int[,] GPat)
        {
        ...
        public Line LinePlotter(Point ptS, Point ptE, Brush br, int wd, int sCode )
        {
            try
            {
                Line ln = new Line();
