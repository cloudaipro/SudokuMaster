...
namespace GNPXcore
{
    public partial class NuPz_Win
    {
        ...
        */
        ...
        {
            //alex if (txbPuzzle == null || GNPX_000.pGNPX_Eng == null) return (false, -1, -1, -1);
            if (GNPX_000.pGNPX_Eng == null) return (false, -1, -1, -1);
            var (confirmedAll, nP, nZ, nM) = pAnMan.Aggregate_CellsPZM(pGP.BDL);

            if (nP + nZ + nM > 0)
            {
                //alex txbPuzzle.Text = nP.ToString();
                //alex txbSolved.Text = nM.ToString();
                //alex txbUnsolved.Text = nZ.ToString();
                //alex if (nP > 0) txbUnsolved.Background = (nZ == 0) ? Brushes.Navy : Brushes.Black;
                //alex
                /* alex
        ...
        */
        ...
        private void btnCopyBitMap_Click(object sender, RoutedEventArgs e)
        {
            ...
            catch (System.Runtime.InteropServices.COMException) { /* NOP //alex
        }
